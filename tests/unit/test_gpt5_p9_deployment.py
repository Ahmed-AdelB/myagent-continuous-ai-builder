...
# Import core deployment classes
from core.deployment.deployment_config import (
    DeploymentStage,
    DeploymentStatus,
    Environment,
    DeploymentStrategy,
...
class MockDeploymentOrchestrator:
    ...
    async def _create_rollback_plan(self, config: DeploymentConfig) -> Dict[str, Any]:
        ...
        try:
            v_str = config.version.lower().lstrip('v')
            parts = v_str.split('.')
            if len(parts) >= 2:
                major = parts[0]
                minor = int(parts[1])
                new_minor = max(0, minor - 1)
                rollback_version = f"v{major}.{new_minor}"
            else:
                rollback_version = "v0.0"
